...
# Tracks exact-duplicate chunk hashes across the whole ingestion run so repeated
# boilerplate/headers/footers in noisy data don't each burn a separate embedding
# call. Reset per process — call run_universal_ingestion.clear() (below) if you
# need a fresh run in the same session.
_seen_chunk_hashes: set[str] = set()


def _dedup_chunks(chunks: list[str], filename: str) -> list[str]:
    unique = []
    skipped = 0
    for c in chunks:
        h = hashlib.sha256(c.strip().encode("utf-8")).hexdigest()
        if h in _seen_chunk_hashes:
            skipped += 1
            continue
        _seen_chunk_hashes.add(h)
        unique.append(c)
    if skipped:
        logfire.info(f"Skipped {skipped} duplicate chunk(s) in {filename} (already embedded this run).")
    return unique
...
def process_file(file_path: str, filename: str, source_type: str):
    ...
    with logfire.span("Processing File", file=filename, source=source_type):
        try:
            ...
            # 2b. Drop exact duplicates before spending embedding-API quota on them
            chunks = _dedup_chunks(chunks, filename)
            if not chunks:
                logfire.info(f"All chunks in {filename} were duplicates — nothing new to embed.")
                return
